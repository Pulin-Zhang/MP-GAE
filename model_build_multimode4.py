...
class PEMFC_model():
    def __init__(self):
        ...
        t_density=(t_density-d_min)/(d_max-d_min)
        te_density=(te_density-d_min)/(d_max-d_min)
        t_y_density=np.clip((t_y_density-d_min)/(d_max-d_min),1e-3,1)
        te_y_density=np.clip((te_y_density-d_min)/(d_max-d_min),1e-3,1)
        d_id = self.dict['density']
        train_env_his[:,:,d_id],train_X_in[:,:,d_id],test_env_his[:,:,d_id],test_X_in[:,:,d_id]=\
            (train_env_his[:,:,d_id]-d_min)/(d_max-d_min),(train_X_in[:,:,d_id]-d_min)/(d_max-d_min),(test_env_his[:,:,d_id]-d_min)/(d_max-d_min),(test_X_in[:,:,d_id]-d_min)/(d_max-d_min)

        v_max = np.max(t_voltage)
        v_min = np.min(t_voltage)
        with open('./{}_Model/epoch{}_max_data.txt'.format('voltage', self.epoch), "w") as file:
            file.write(str(v_max))
        with open('./{}_Model/epoch{}_min_data.txt'.format('voltage', self.epoch), "w") as file:
            file.write(str(v_min))
        t_voltage=(t_voltage-v_min)/(v_max-v_min)
        te_voltage=(te_voltage-v_min)/(v_max-v_min)
        t_y_voltage=np.clip((t_y_voltage-v_min)/(v_max-v_min),1e-3,1)
        te_y_voltage=np.clip((te_y_voltage-v_min)/(v_max-v_min),1e-3,1)
        ...
        history=pemfc_model.fit(x=[t_temperature,t_humidity,t_hfr,t_density,t_voltage,train_env_his,train_X_in],
            y=[t_y_temperature,t_y_humidity,t_y_hfr,t_y_density,t_y_voltage], validation_data=([te_temperature,te_humidity,te_hfr,te_density,te_voltage,test_env_his,test_X_in],
                    [te_y_temperature,te_y_humidity,te_y_hfr,te_y_density,te_y_voltage]),epochs=self.epoch, batch_size=self.batchsize,verbose=2)
    ...
    def PEMFC_build(self):
        ...
        T_encode_SA=MultiHead_Attention(head_num=4,activation=None,name='T_encode_SA')
        T_x =tf.nn.sigmoid(T_encode_SA(T_x))
        ...
        H_encode_SA=MultiHead_Attention(head_num=4,activation=None,name='H_encode_SA')
        H_x =tf.nn.sigmoid(H_encode_SA(H_x))
        ...
        Hfr_encode_SA=MultiHead_Attention(head_num=4,activation=None,name='Hfr_encode_SA')
        hfr_x =tf.nn.sigmoid(Hfr_encode_SA(hfr_x))
        hfr_code = Conv1D(filters=1,kernel_size=1,activation='relu',
                            input_shape=(self.feature_num,self.feature_num))(hfr_x)

        ### density_encode_model
        density_his_data = Input(shape=(self.time_read, self.feature_num))
        d_id = int(self.dict['density'])
        d_env_his = env_his_data[:,:,d_id]
        d_env_his = tf.tile(tf.expand_dims(d_env_his, axis=-1),[1,1,40])
        d_single = env_data[:,:,d_id]
        d_single = tf.tile(tf.expand_dims(d_single, axis=-1),[1,1,40])
        GAT_layer_d_encode = GraphAttention(num_feature=40,name='{}'.format('d_encode'),heads=4)
        d_x,_ = GAT_layer_d_encode(d_env_his-density_his_data,self.ca_adj)
        d_encode_SA=MultiHead_Attention(head_num=4,activation=None,name='d_encode_SA')
        d_x =tf.nn.sigmoid(d_encode_SA(d_x))
        d_code = Conv1D(filters=1,kernel_size=1,activation='relu',input_shape=(self.feature_num,self.feature_num))(d_x)

        ### voltage_encode_model
        voltage_his_data = Input(shape=(self.time_read, self.feature_num))
        GAT_layer_v_encode = GraphAttention(num_feature=40,name='{}'.format('v_encode'),heads=4)
        v_x,_ = GAT_layer_v_encode(voltage_his_data,self.ca_adj)
        v_encode_SA=MultiHead_Attention(head_num=4,activation=None,name='v_encode_SA')
        v_x =tf.nn.sigmoid(v_encode_SA(v_x))
        v_code = Conv1D(filters=1,kernel_size=1,activation='relu',
                            input_shape=(self.feature_num,self.feature_num))(v_x)

        code = tf.concat([T_code,H_code,hfr_code,d_code,v_code],-1)
        ...
        ### Density_Decoder
        D_decode = Dense(self.feature_num*2)(code)
        D_decode = Dense(self.feature_num)(D_decode)
        D_gate_layer=MultiHead_Attention(head_num=4,activation=None,name='D_gate_layer')
        D_gate =tf.nn.sigmoid(D_gate_layer(D_decode))
        d = LSTM(units=self.feature_num,input_shape=(self.time_read,self.feature_num),
                            return_sequences=True)(D_decode*D_gate)
        GAT_layer_D_decode = GraphAttention(num_feature=40,name='{}'.format('D_decode'),heads=4)
        d,_ = GAT_layer_D_decode(d,self.ca_adj)
        d = tf.transpose(d,perm=[0,2,1])
        D_predict_result =Conv1D(filters=1,kernel_size=1,activation='tanh',
                            input_shape=(self.feature_num,self.feature_num))(d)
        D_predict_result = tf.transpose(D_predict_result,perm=[0,2,1])
        density_result=Lambda(lambda x:x[0]+x[1],name='Density')([D_predict_result,d_single])

        ### voltage_Decoder
        voltage_decode = Dense(self.feature_num*2)(code)
        voltage_decode = Dense(self.feature_num)(voltage_decode)
        voltage_gate_layer=MultiHead_Attention(head_num=4,activation=None,name='voltage_gate_layer')
        voltage_gate =tf.nn.sigmoid(voltage_gate_layer(voltage_decode))
        voltage =LSTM(units=self.feature_num,input_shape=(self.time_read,self.feature_num),
                            return_sequences=True)(voltage_decode*voltage_gate)
        GAT_layer_voltage_decode = GraphAttention(num_feature=40,name='{}'.format('voltage_decode'),heads=4)
        voltage,_ = GAT_layer_voltage_decode(voltage,self.ca_adj)
        voltage = tf.transpose(voltage,perm=[0,2,1])
        voltage_predict_result =Conv1D(filters=1,kernel_size=1,activation='sigmoid',
                            input_shape=(self.feature_num,self.feature_num))(voltage)
        voltage_result = tf.transpose(voltage_predict_result,perm=[0,2,1])
        voltage_result =Lambda(lambda x:x[0]+(x[1]-x[1]),name='Voltage')([voltage_result,T_single])

        return Model(inputs=[Temperature_his_data,Humidity_his_data,hfr_his_data,density_his_data,voltage_his_data,env_his_data,env_data],
                     outputs=[T_result,H_result,hfr_result,density_result,voltage_result])
    ...
    def plot_history(self,history,name):
        ...
        plt.savefig('./demo_result/{}__Model{}_{}_{}_Acc.png'.format(self.mode, self.version, name, self.epoch),bbox_inches='tight')
        ...
        plt.tick_params(labelsize=20)
        plt.xlabel('Epoch',font)
        plt.ylabel('Mean Square Error Loss',font)
        plt.legend(loc='best',prop={'size':20})
        plt.savefig('./demo_result/{}__Model{}_{}_{}_MSE.png'.format(self.mode, self.version, name, self.epoch),bbox_inches='tight')
